...
class RegisterController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        if ($firstName === '' || $lastName === '') return $this->error('First name and last name are required', 422);
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) return $this->error('Invalid email address', 422);
        if (strlen($password) < 8) return $this->error('Password must be at least 8 characters', 422);
        if (!in_array($roleSlug, ['tenant', 'owner'], true)) return $this->error('Invalid registration role', 422);
        if (DB::table('users')->where('email', $email)->exists()) return $this->error('Email already registered', 409);
        if ($phone !== '' && DB::table('users')->where('phone', $phone)->exists()) return $this->error('Phone already registered', 409);
        ...
        try {
            ...
            DB::table('user_profiles')->insert(['user_id' => $userId, 'preferred_language' => 'en', 'currency' => 'AED']);
            ...
            $referralCode = strtoupper('VIBE-' . $userId . '-' . bin2hex(random_bytes(3)));
